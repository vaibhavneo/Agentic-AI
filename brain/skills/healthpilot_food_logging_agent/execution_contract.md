# Execution contract — healthpilot_food_logging_agent v1.0.0
...
1. **ROUTE TO SPECIALIST** — the adapter maps this skill's id
   (`healthpilot_food_logging_agent`) to HealthPilot's own internal specialist key
   (`food_logging_agent`, from `agents/specialists.py::SPECIALISTS`) and sends
...
This skill answers as the **Food Logging Agent** specialist specifically.
